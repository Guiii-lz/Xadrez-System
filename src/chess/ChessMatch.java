...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		if(promoted == null) {
			throw new IllegalStateException("There's not piece to promoted");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		// Movimento especial Rook - small rook
		...
		// Movimento especial Rook - big rook
		if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("A Peça escolhida não pode se mover para o lugar desejado");
		}
	}
